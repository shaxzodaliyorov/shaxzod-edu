...
export const getServerSideProps: GetServerSideProps<LessonPropsType> = async ({
  req,
  query,
}) => {
  const user = await USER.GetMyUser(req.cookies.token as string);
  const Lessons = await Lesson.getLessons(query.courseid as string);
  const course = await GET_ALL_COURSES.GET_ONE(query.lesson as string);
